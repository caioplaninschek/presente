...
- **R44 O buzzer entra por transistor, e a peça atual precisa ser trocada.** Fecha a checagem da R22. O João ligou o buzzer por um transistor NPN Darlington **TIP122**, com a peça alimentada pelos 5 V do pino VIN e o sinal saindo do GPIO 33. Com o programa da prova de vida, as três cores do LED bateram com o monitor serial, e o bip saiu muito baixo, com um som estranho, audível só de perto. Para o João, a peça está ruim e precisa ser comprada de novo; para a demonstração de 22/09, ela serve de perto. **Consequências:**
  ...
  - **Na demonstração de 22/09, o professor perguntou se o buzzer era ativo ou passivo** e sugeriu que a causa fosse a alimentação em 5 V, a resolver alimentando a peça com uma tensão menor. O João respondeu que a peça, ligada à parte num Arduino, em 5 V, também não funcionou, e que a hipótese dele continua sendo o disco cerâmico danificado. A conferência ativo ou passivo segue sem fazer.
...
|---|---|---|
| Testes **3, 4, 5 e 6** | Todos dependem do roster **dentro** do aparelho, que só existe quando o ESP32 falar com a nuvem. Vivem no epic #9, ainda não fragmentado. Declarado na #28, mas nenhum ticket os possui | Na quebra do epic #9 (dashboard), cuja data o professor ainda vai confirmar |
...
| **`sessao_atual.json`** (R16) | Requisito explícito da spec sem ticket — a #27 cobre só o `eventos.json` | Com a sessão de verdade, depois do portal (depois da entrega do dashboard) |
| **Máquina de estados completa** do §5 (`AGUARDANDO_LOGIN`, `SINCRONIZANDO`, `SESSAO_ABERTA`) | As tasks de 22/09 cobrem só `REGISTRADO`, `JA_REGISTRADO` e o feedback; os estados de rede e de sessão dependem do portal (#20). O veredito de rádio (#19) saiu em 22/09, e o `FALHA_DE_RADIO` caiu com ele (R45) | Na integração, depois da Entrega 05 |
