...
- **Troca da senha padrão no primeiro uso** (HU-04) e **configuração do hotspot pelo portal** (HU-15), pela R47: as contas chegam cadastradas pelo grupo, e o hotspot e o de reserva vêm do `secrets.h`.
- **Escolha da turma no aparelho**, pela R48: a turma e a disciplina ficam fixas na configuração; em produção, viriam da grade horária da instituição.
...
4. **HU-04** Como professor, quero trocar a senha padrão no primeiro uso, para que minha conta não fique com a senha de fábrica. *(cortada em R47, 23/09: as contas chegam cadastradas pelo grupo)*
...
6. **HU-06** Como professor, quero ver pelo LED e pela tela que o aparelho está sincronizando, para não achar que travou enquanto ele sincroniza. ~~O que no Plano B inclui os 5–15 s da troca de rádio~~: vale o Plano A, e o celular não sai da rede do aparelho (R45, 22/09).
...
15. **HU-15** Como professor, quero configurar o SSID e a senha do hotspot pelo próprio portal, para trocar de celular sem reprogramar o aparelho. *(cortada em R47, 23/09: o hotspot e o de reserva vêm do `secrets.h`, HU-29)*
...
- **R13d Checklist de hotspot** (vai impresso para a apresentação):
  ...
  - Abrir o painel do Supabase antes do dia e, se o projeto estiver pausado, retomá-lo (Resume project). Projeto gratuito pausa depois de 7 dias com pouco uso, e com o projeto pausado o login do aparelho cai no modo offline sem aviso (R33, 13/09). ⚠️ Desde a R50 (23/09), um agendamento no GitHub (tarefa 5.1) mantém o projeto acordado; a conferência continua no checklist.

### Rodada 5 — R14–R17 (06/09, 01h)

- **R14 A sincronização acontece no login, não no Iniciar.** O aparelho valida a senha, responde *"carregando turma…"*, e só então troca o rádio. Quando o painel abre, o roster está em cache e o **Iniciar** é instantâneo. A espera de 5–15 s não some — muda para o instante em que o professor já espera que a página demore. ⚠️ **Com R45 (22/09)** o rádio não troca mais: a espera passa a ser só a da busca da turma pelo hotspot, com o celular do professor ainda conectado. Altera o §6.
...
- **R15 Recuperação quando o AP não volta:** ⚠️ **deixou de valer em R45 (22/09):** no Plano A o AP nunca é desligado, e não há volta que possa falhar. O texto fica como registro. 2 tentativas (nunca mais, por causa do vazamento de R13b), depois `ESP.restart()`. O LED distingue três estados de rede: azul lento = sincronizando; azul rápido = vai reiniciar; azul fixo = de pé, aguardando login.
- **R16 `sessao_atual.json` é requisito, não conveniência.** Gravado antes de toda troca de rádio, lido no boot. ⚠️ **Com R45 (22/09)** não há mais troca de rádio: o arquivo é gravado quando a sessão abre, e continua sendo requisito por causa da queda de energia. Reinício no meio da aula retoma a sessão em curso (mesmo `professorId`, mesmo roster, janela ainda aberta) em vez de cair no login com presenças órfãs. É o que torna R15 aceitável.
...
- **R19 O modo `WIFI_AP_STA` existe e é nativo; a escolha entre simultâneo e alternado vira medição.** O João trouxe a correção na issue #3 e ela procede — **R4 estava errado na premissa**. A documentação da Espressif (guia de Wi-Fi do ESP-IDF) diz que os dois modos coexistem, com uma restrição: `"In Station/AP-coexistence mode, the home channel of AP and station must be the same, if they are different, the station's home channel is always in priority."` O softAP acompanha o canal do STA e anuncia a troca por **CSA**; estação que suporta channel switch acompanha **sem cair**. Ou seja, o canal para de ser impedimento — o que sobra como risco é a memória (R13a). O projeto passa a ter dois planos:
  ...
  - **Plano B — alternância `AP → STA → AP`.** O que R4 especifica hoje. Só entra se o A reprovar. ⚠️ **Não entrou:** o A passou (R45, 22/09).
...
- **R32 O veredito parcial adota o Plano A provisoriamente.** Se o 7c sair parcial (R24) e aprovado, e a memória passar, a spec adota o Plano A para a integração e registra no R19 qual sistema de celular faltou. Esse sistema é testado até a entrega do dashboard (05/10 pelo cronograma de 05/09): se reprovar, o projeto vai para o Plano B, que continua especificado; se o teste não acontecer até lá, a apresentação usa um celular do sistema já aprovado.
  ...
  - ⚠️ **Não precisou ser usado:** o teste do painel de 22/09 cobriu iPhone e Android, e o veredito saiu completo (R45).
...
- **R33 O aparelho só entra no banco por duas funções.** A chave gravada no aparelho não tem privilégio em tabela nenhuma. Ela executa `roster(turma)`, que devolve matrícula, nome e UID dos alunos ativos de uma turma, e `enviar_relatorio(relatorio)`, que recebe o JSON do Compartilhar (§7) e grava sessão e eventos numa transação. As duas moram no próprio Postgres, e o `PostgREST` as expõe em `/rest/v1/rpc/`, então continua não havendo servidor para escrever (R21). O SQL fica versionado em `supabase/`, em arquivos numerados aplicados pelo editor de SQL do painel; arquivo aplicado não se edita, e mudança vira o arquivo seguinte. **Consequências:**
  ...
  - **Dados de teste.** São inventados, salvo o nome e a matrícula dos cinco integrantes, e as aulas passadas têm id começando em `teste-`. Saem na integração, antes do primeiro relatório de aula real, junto com as sessões `verificacao-` que o `supabase/verificar.ps1` deixa a cada rodada. ⚠️ A limpeza é a tarefa 5.17 (rodada 16, 23/09).
...
- **R44 O buzzer entra por transistor, e a peça atual precisa ser trocada.** Fecha a checagem da R22. O João ligou o buzzer por um transistor NPN Darlington **TIP122**, com a peça alimentada pelos 5 V do pino VIN e o sinal saindo do GPIO 33. Com o programa da prova de vida, as três cores do LED bateram com o monitor serial, e o bip saiu muito baixo, com um som estranho, audível só de perto. Para o João, a peça está ruim e precisa ser comprada de novo; para a demonstração de 22/09, ela serve de perto. **Consequências:**
  ...
  - **Na demonstração de 22/09, o professor perguntou se o buzzer era ativo ou passivo** e sugeriu que a causa fosse a alimentação em 5 V, a resolver alimentando a peça com uma tensão menor. O João respondeu que a peça, ligada à parte num Arduino, em 5 V, também não funcionou, e que a hipótese dele continua sendo o disco cerâmico danificado. A conferência ativo ou passivo segue sem fazer. ⚠️ **Dispensada em R54 (23/09):** a compra é de buzzer ativo de 5 V nos dois casos.
...
### Rodada 15 — R45 (22/09) — o veredito do rádio

Nasce do teste do painel do professor (#18), feito na sala depois da demonstração da Entrega 05, e fecha o veredito (#19). O resultado chegou pelo relato do Caio, em 23/09.

- **R45 Vale o Plano A: o aparelho mantém a própria rede no ar enquanto usa o hotspot.** Os dois critérios da R19 passaram. A memória passou em 21/09, com 236,5 KB livres antes da conexão segura (#17) e 229,6 KB com um celular conectado (#18), contra os 40 KB do critério. O painel passou em 22/09, na sala: com o Android do João servindo de hotspot e o iPhone do Cauã no papel do professor, e depois com os papéis trocados, duas rodadas por celular. Nas quatro, a página continuou no painel sem pedir login de novo, e nenhum celular saiu da rede do aparelho. O veredito é completo, porque os dois sistemas foram testados, e a R32 não precisou ser usada. **Consequências:**
  - O R4 fica encerrado como alternativa derrubada, e saem do projeto a alternância `AP → STA → AP`, a espera de 5 a 15 s da troca de rádio, a recuperação do R15 e o estado `FALHA_DE_RADIO`. O R16 continua, por causa da queda de energia.
  - O §3, o §4, o §5, o §6, o §8, o §10, a HU-06 e o glossário passam a descrever um plano só. O fluxograma da lógica perde os passos que eram marcados com `*`, e o rótulo do rádio no diagrama de blocos passa a dizer "ao mesmo tempo, no mesmo canal". Os PDFs já entregues não mudam.
  - Os testes 7 e 7b deixam de existir, e a lacuna deles no §8 fecha.
  - ⚠️ **A evidência é mais fraca do que o §8 pede.** Não há trecho do monitor serial nem foto das rodadas, e o resultado está registrado pelo relato do grupo. Também não ficou anotado que a linha `Canal da rede do aparelho: antes X, agora Y` mostrou números diferentes, que é o que prova o pulo de canal, nem como o `secrets.h` do João, que tinha o hotspot do Android, foi ajustado para a rodada com os papéis trocados. O roteiro usado na sala pedia a conferência do pulo em cada rodada, e o Caio adotou o resultado como definitivo em 23/09. Se na integração o painel cair quando o aparelho entrar no hotspot, é este teste que se refaz, com o log.
  - O que o teste só observava continua em aberto: onde a página de login abre e se o celular troca sozinho para o 4G (lacuna do §8, perguntas 2 e 3 do portal cativo).

### Rodada 16 — R46–R55 (23/09) — o plano até a entrega final

Nasce da sessão de planejamento de 23/09, feita depois da demonstração aprovada, para dizer o que falta até o projeto funcionar de ponta a ponta. As lacunas do §8 recebem destino, a caixa e as compras se decidem, e o grupo muda a forma de dividir o trabalho. As decisões de produto e de time são do Caio; as técnicas foram propostas na mesma sessão e aceitas por ele.

- **R46 O Enviar bem-sucedido fecha a chamada.** O botão passa a se chamar *Enviar e fechar a chamada*. Com a resposta `gravado` ou `ja_enviado`, o aparelho vai para `CHAMADA_ENVIADA`: o LED fica azul fixo, o leitor deixa de registrar, e o professor ainda pode exibir e compartilhar o relatório antes de encerrar. O crachá encostado nesse estado recebe o sinal de `NAO_RECONHECIDO`, vermelho por 1 s com buzz, e nada é gravado; o aluno atrasado entende que precisa procurar o professor (HU-20). O sinal vale nos dois modos, porque depois do Enviar também o modo offline não tem mais onde gravar. Com o Enviar falho, a chamada continua em `SESSAO_ABERTA`, registrando, e o Compartilhar fica disponível. **Por quê:** o banco recusa o segundo envio da mesma aula (R33), e um toque gravado depois do primeiro nunca chegaria a ele; o aparelho diria ao aluno que registrou, e o registro oficial diria que não. A mesma decisão fecha o outro lado da lacuna: depois do login e da sincronização, o aparelho espera o Iniciar em `AGUARDANDO_INICIAR`, com o LED azul fixo. Fecha a lacuna "estado depois do login e do Enviar" do §8.
  - ~~A chamada continua aberta depois do Enviar, e o toque grava só no aparelho~~: o que o aluno vê e o que fica registrado divergiriam sem aviso (23/09).
  - ~~O banco aceita reenvio e soma os toques novos~~: mudaria a função `enviar_relatorio` e o teste de duplicidade (V5) por um caso que o nome do botão resolve (23/09).
  - ~~O toque depois do Enviar é ignorado em silêncio~~: sem sinal nenhum, o aluno acharia que o aparelho travou (23/09).
  - ~~Verde de uma piscada, como `JA_REGISTRADO`~~: o aluno atrasado sentaria achando que foi registrado (23/09).
- **R47 A troca de senha no primeiro uso (HU-04) e a configuração do hotspot pelo portal (HU-15) saem do núcleo.** O critério é do Caio: núcleo completo, com os cortes declarados. As duas histórias acrescentam telas e escrita no LittleFS sem mudar o que a chamada faz. As contas chegam cadastradas pelo grupo no `data/professores.json`, com hash e salt (Q4), e o hotspot e o de reserva continuam no `secrets.h` (HU-29). As outras 38 histórias continuam no núcleo.
- **R48 A turma e a disciplina ficam fixas na configuração do aparelho.** Os valores são `4172CMPN6A_P1` e `Sistemas Embarcados`, no `config.h`, e o portal só os exibe. Em produção, a turma viria da grade horária da instituição, e o corte fica declarado no §1. Fecha a lacuna "de onde vem a turma no login".
  - ~~O professor escolhe a turma no login~~: exigiria uma função nova no banco e uma tela nova, para um protótipo com uma turma só (23/09).
  - ~~A turma deduzida do professor e do horário~~: depende de uma grade horária que o projeto não tem (23/09).
- **R49 O login fica na janela do portal cativo, e um novo login retoma a chamada em curso.** É a opção (a) do ticket `.scratch/presente/issues/09`. A chamada vive no aparelho, no `sessao_atual.json` (R16), e não no celular: se o sistema fechar a janela e o cookie se perder, o professor entra de novo e volta ao painel da chamada aberta. O portal pergunta `/api/estado` ao abrir, antes de mostrar o login. A pergunta 3 do ticket, sobre o celular que vê "rede sem internet" e troca sozinho para o 4G, fica como observação obrigatória na placa, em iPhone e Android, nas tarefas 5.8 e 5.16. Se o painel cair por esse motivo, a decisão se reabre com o log.
  - ~~(b) A janela só traz o professor, e ele segue em `192.168.4.1` no navegador~~: com a retomada pelo login, o cookie perdido custa só digitar a senha de novo (23/09).
- **R50 A página de histórico sai pelo GitHub Pages, e um agendamento mantém o banco acordado.** A página da R21 e da R34 é publicada por um workflow do GitHub Actions, que monta a configuração com a chave publicável a partir de um segredo do repositório; a chave não entra no git, e a regra do `AGENTS.md` continua de pé. **Custo assumido, a declarar no relatório técnico:** a chave fica visível para quem abrir a página. É o mesmo custo da R33, agora ao alcance de qualquer um com o endereço, e a leitura do histórico continua exigindo login (R34). Um segundo workflow chama `rpc/roster` da turma `TESTE01` a cada três dias, porque o projeto gratuito pausa depois de cerca de sete dias sem uso e, pausado, derruba o login do aparelho no modo offline sem aviso (R13d). Os segredos dos dois workflows são postos pelo Caio.
- **R51 O firmware integrado se constrói em cinco pacotes, cada um escrito numa issue e rodado na placa em outra.** O código mora em `src/presente/`, com o ambiente `presente` (extensão da R28), e reúne o `config.h`, o `portal.cpp`, o `net.cpp`, o `clock.cpp` e o `main.cpp` do §5; as bibliotecas de `lib/` evoluem no lugar. Os pacotes são cinco: o portal dentro do aparelho; a lista da turma pela internet; a chamada; o relatório; e a queda de energia com a rodada completa do §8. Cada pacote só vai para a placa depois que alguém que não o escreveu revisa o código e compila na própria máquina, porque a placa não circula (R23) e cada rodada na placa custa cerca de um dia. O `data/professores.json` real fica fora do git, com um `professores.example.json` versionado e um script que gera o hash. O `data/mentira.js` continua, para o portal ser desenvolvido sem placa, mas só carrega com `?mentira=1` no endereço.
- **R52 O trabalho é livre.** Decisão do Caio: nenhuma tarefa é designada, e cada integrante pega as issues que quiser. Cinco regras sustentam isso. Pegar é atribuir a issue a si e comentar antes de começar, uma pessoa por issue ou uma dupla combinada no comentário. Cada issue diz o que exige numa linha *Requer*: a placa, acesso ao Supabase, ser admin do repositório ou nada. Issue com prazo e sem ninguém a dois dias do vencimento é chamada no grupo pelo Caio, que continua como PO sem designar. No firmware, quem revisa não é o autor (R51). A epic mostra quem pegou cada tarefa. **Exceção, os donos naturais:** nascem com dono as issues que dependem de acesso ou de trabalho já em andamento. O Caio fica com o que mexe no banco, porque criou o projeto no Supabase e é o único com acesso; o João fica com a compra do buzzer e dos cartões, porque a placa está com ele; o Igor continua com o portal e com a página de histórico, que já eram dele (#20, #29, R21). **Consequência no §9:** nem a tabela de papéis nem as atribuições por issue valem como contrato, e a divergência está declarada ao pé da tabela. A R23 continua: a placa não circula, e as tarefas de rodar na placa exigem quem estiver com ela.
  - ~~Cada frente com um dono designado pelo Caio~~: o Caio não quer distribuir tarefa, e o interesse de cada um rende mais que a designação (23/09).
- **R53 A caixa é de papelão ou de material reaproveitado, feita pelo grupo, com um desenho.** O custo precisa ficar perto de zero, e o professor aceitou um desenho de como o aparelho fica montado. Cada integrante propõe uma ideia até 01/10, ~~o Caio escolhe uma, ou junta duas, em 02/10~~ o grupo escolhe junto uma, ou junta duas, em 02/10, e o Caio decide se ninguém responder até lá *(ajustado em 23/09)*, e a montagem com a placa acontece em 03 e 04/10, seguida de um teste com a caixa fechada. **As restrições das antenas:** o RC522 fica colado atrás da tampa da frente, que ali tem no máximo 2 a 3 mm, sem metal nenhum por perto e longe da protoboard e dos fios; a ponta do ESP32 onde fica a antena de Wi-Fi fica livre e, se possível, voltada para fora. Fecha o ticket `.scratch/presente/issues/04`.
  - ~~Impressão 3D~~: ninguém do grupo modela em CAD, e não há impressora disponível (23/09).
  - ~~MDF cortado em papelaria, com parafusos~~: custa e exige ferramenta, e o papelão atende (23/09).
  - ~~A caixa com o João como dono~~: a caixa é de todos (R52, 23/09).
- **R54 Compras: buzzer ativo de 5 V e cartões NTAG215, e os cartões novos entram no banco.** O João compra dois buzzers ativos de 5 V, um deles de reserva, e cerca de dez cartões NTAG215, e o grupo divide o custo. A conferência ativo ou passivo da R44 fica dispensada: a peça atual é passiva ou está danificada, e a compra é a mesma nos dois casos. Nada muda na ligação pelo TIP122 nem no firmware, que liga o buzzer com nível alto constante. Os cartões têm o mesmo chip da tag adesiva, com antena maior, que o RC522 detecta com mais folga, e fecham o teste 2 do §8, que saiu em parte na Entrega 05. Os números dos cartões entram por `supabase/003-cartoes.sql`, ligados aos cinco integrantes (R1), e um cartão fica sem cadastro para o teste 3.
- **R55 O que vai em cada entrega.** O portal servido pelo ESP32 é o "dashboard com ESP32" que o enunciado pede, e a página de histórico é o complemento na nuvem. Na entrega do dashboard vão a página de histórico e o portal no aparelho, no pacote em que estiver. O firmware completo mira a entrega do protótipo completo, e não a do dashboard, para que uma rodada atrasada na placa não derrube a entrega mais próxima. As datas continuam as dos enunciados (`docs/entregas.md`).
...
[Tag NFC 13,56 MHz] --RF--> [RC522] --SPI VSPI--> [ESP32 DevKit V1]
                              ...
                              ao mesmo tempo, no mesmo canal (Plano A) — R45
                                            |                       |
...
|---|---|---|---|
| ESP32 DevKit V1 / 32D | 1 | Controlador principal, Wi-Fi (AP+STA, R45), WebServer, LittleFS | ✅ adquirido |
...
| Buzzer ativo 5V | 1 | **Atuador 2** — bip 100 ms na confirmação (GPIO 33) | ⚠️ montado; som fraco. A troca é por buzzer ativo de 5 V, comprado sem a conferência ativo ou passivo (R54, tarefa 5.5) |
| Transistor NPN TIP122 | 1 | Chaveia o buzzer de 5 V a partir do GPIO 33 (R44) | ✅ montado em 21/09 |
| Cartões NTAG215 | ~10 | Crachás de teste com antena maior, que o RC522 lê com mais folga que a tag adesiva; fecham o teste 2 (R54) | ⬜ compra na tarefa 5.5 |
...
✅ **Lista fechada em 10/09/2026 (R22):** o João comprou tudo e as peças estão com ele. ~~Não há compra pendente.~~ ⚠️ **Uma compra reabriu em 21/09 (R44):** um buzzer novo, depois de conferir se a peça atual é passiva. Em 23/09 (R54), a conferência foi dispensada, e a compra ganhou os cartões NTAG215.
...
Até 22/09, os programas de bancada moram cada um na sua pasta de `src/`, com o seu ambiente (R28). A tabela abaixo descreve o firmware de verdade, que mora em `src/presente/`, com o ambiente `presente`, e se constrói em cinco pacotes (R51).
...
| `net.cpp` | Rádio: `WIFI_AP_STA`, com o AP sempre no ar (R45). ~~No Plano B, alternância AP↔STA (sequência fixa: parar server+DNS → `softAPdisconnect(true)` → `WIFI_OFF` → `delay(500)` → `WIFI_STA`; e o inverso na volta)~~: derrubado em R45 (22/09). Roster, upload, retry **com limite** |
...
| `SINCRONIZANDO` | azul lento | — | o aparelho falando com a API pelo hotspot (após o login, ou no Enviar) |
| `AGUARDANDO_INICIAR` | azul fixo | — | a sincronização do login terminou, com ou sem a lista (R46) |
| ~~`FALHA_DE_RADIO`~~ | ~~azul rápido~~ | — | ~~AP não voltou em 2 tentativas → `ESP.restart()` (só no Plano B)~~: sai com o Plano B (R45, 22/09) |
...
| `NAO_RECONHECIDO` | vermelho 1 s | buzz 400 ms | **só em modo online**: UID fora da lista da turma — nada é gravado; e todo toque depois do Enviar, nos dois modos (R46) |
| `CHAMADA_ENVIADA` | azul fixo | — | Enviar bem-sucedido (R46): o leitor deixa de registrar, e o professor ainda exibe e compartilha o relatório |

Em **modo offline**, enquanto a chamada está aberta, o estado `NAO_RECONHECIDO` não existe: sem lista da turma, todo UID lido é aceito e vira `REGISTRADO`. O LED verde passa a significar *"li a tag"*, não *"você está nesta turma"*. Depois do Enviar, todo toque recebe o sinal de `NAO_RECONHECIDO`, nos dois modos (R46).
...
ENQUANTO ligado:                                                // loop(), sem delay()
  ...
  SE o professor autenticou:
      ...
      baixar o roster da turma pelo hotspot, com o AP no ar      // R45
      SE conseguiu:  gravar o roster em cache;  modo <- online
      SENÃO:                                     modo <- offline // R3
      abrir o painel com a lista já em cache                    // R46
      estado <- AGUARDANDO_INICIAR                              // azul fixo até o Iniciar

  SE o professor clicou Iniciar:
      abrir a sessão e gravar sessao_atual.json
      estado <- SESSAO_ABERTA                                   // LED verde fixo

  SE o RC522 leu uma tag nova E a chamada já foi enviada:           // R46
  ...
  SE o professor clicou Enviar:
      ...
      SE falhou: manter eventos.json, continuar em SESSAO_ABERTA e deixar o Compartilhar disponível
      SE deu certo (gravado ou ja_enviado): estado <- CHAMADA_ENVIADA   // R46: a chamada fecha
...
5. Professor entra com login e senha. Validada a senha, o aparelho responde *"carregando turma…"* e **aproveita esse instante para sincronizar** (R14): o STA busca o roster pelo hotspot com o AP no ar, o celular não perde a conexão e o LED pisca azul lento (Plano A, R45). ~~No Plano B, o rádio vai a STA, busca o roster e volta a AP em 5–15 s, e o celular reconecta sozinho~~: derrubado em R45 (22/09)
...
- **O ambiente de teste é a bancada, com o aparelho real.** Não há teste automatizado, nem em host (`pio test` com ambiente `native`). É escolha, não omissão: o que pode dar errado aqui é físico e de rádio — a leitura SPI do RC522, o comportamento de cada celular no pulo de canal do `WIFI_AP_STA` (R19), os 40–50 KB de heap do handshake TLS, a integridade do LittleFS numa queda de energia. Nada disso aparece contra um mock; o teste de host testaria o mock, e a camada de abstração necessária para escrevê-lo custaria mais do que a bancada custa.
...
- **Todo teste entrega evidência gravada** — foto ou vídeo curto do LED e do buzzer, o JSON exportado, ou o trecho do log serial com horário. É o que sustenta o entregável de 22/09 ("testes documentados") e o relatório de 30/11. A evidência **mora no repositório**, em `docs/assets/testes/`, consolidada na seção de testes do rascunho de cada entrega (R27, corrigida em 22/09): o que fica só em comentário de issue some da vista do professor.
- **O log serial com níveis (§5) é instrumento de medição**, não resto de depuração: sem ele, os <200 ms e o diagnóstico do rádio viram opinião.
...
- **Quem fecha uma frente roda os testes que a tocam**; a rodada completa acontece no pacote 5 do firmware (tarefa 5.16, R51) e se repete antes de cada marco com demonstração ao vivo (o protótipo completo e 27/10).
- **Calibração faz parte do teste, não do conserto.** Os tempos (janela de silêncio de 5 s, bip de 100 ms, piscada do `JA_REGISTRADO`, vermelho de 1 s) e o limite da espera no login são valores de partida. Espera-se ajustá-los com o aparelho montado na caixa e alimentado pela fonte definitiva — o teste é o que diz para onde ajustar.
...
| ~~7~~ | ~~**Plano B** — alternância AP↔STA, 10 ciclos seguidos~~ | ~~AP volta nas 10 vezes; celular reconecta; transição dentro de 5–15 s~~: sai com o Plano B (R45, 22/09) |
| ~~7b~~ | ~~**Plano B** — falha forçada na volta ao AP (hotspot desligado no meio)~~ | ~~aparelho se recupera sozinho e **não perde** as presenças já registradas~~: sai com o Plano B (R45, 22/09) |
| 7c | **Plano A** — `WIFI_AP_STA`: professor logado no portal, STA conecta no hotspot em outro canal | painel continua aberto **sem novo login**, em iPhone **e** Android; se cair, reconecta sozinho em <5 s. Com dois celulares do mesmo sistema, roda assim mesmo: o veredito sai **parcial** e o outro sistema fecha depois (R24); parcial e aprovado, adota o Plano A provisoriamente (R32). ✅ **Passou em 22/09, com iPhone e Android, pelo relato do grupo e sem log (R45)** |
| 7d | **Plano A** — heap livre no instante do handshake TLS, com AP + DNS + WebServer + roster de pé | ≥40 KB livres (`ESP.getFreeHeap()` logado antes do `connect()`); abaixo disso, reprova e vale o Plano B. Medido com um celular só, sem cliente no AP, o número sai otimista — **40 a 45 KB é suspeito, não aprovado** (R26), e a memória que decide passa a ser a medida no 7c, com cliente conectado (R31). ✅ **Passou em 21/09: 236,5 KB, e 229,6 KB com cliente conectado** |
...
**Lacunas conhecidas (11/09, ampliadas em 13/09 e em 15/09).** ⚠️ **Na rodada 16 (23/09), todas ganharam destino**, anotado na última coluna: uma decisão da rodada ou uma tarefa das epics #8 e #9. Dez itens **não têm task** e não vão ter antes da Entrega 05 — nenhum cabe na janela. Ficam listados para que a ausência seja escolha declarada, e não descuido:

| Item sem dono | Por que não virou task | Quando vira |
|---|---|---|
| Testes **3, 4, 5 e 6** | Todos dependem do roster **dentro** do aparelho, que só existe quando o ESP32 falar com a nuvem. Vivem no epic #9, ainda não fragmentado. Declarado na #28, mas nenhum ticket os possui | Na quebra do epic #9 (dashboard), cuja data o professor ainda vai confirmar. **Rodada 16 (23/09):** teste 3 na tarefa 5.12; testes 4, 5 e 6 na tarefa 5.14 |
| ~~Testes **7 e 7b** (Plano B)~~ | ~~Só se tornam obrigatórios se a #19 der veredito **Plano B**; hoje a #19 emite o veredito e para aí~~ | ~~No dia do veredito, se der Plano B~~: fechada em 22/09, porque o veredito foi Plano A (R45) |
| **`sessao_atual.json`** (R16) | Requisito explícito da spec sem ticket — a #27 cobre só o `eventos.json` | Com a sessão de verdade, depois do portal (depois da entrega do dashboard). **Rodada 16:** gravado na tarefa 5.11, retomado na 5.15 e conferido na 5.16 |
| **Máquina de estados completa** do §5 (`AGUARDANDO_LOGIN`, `SINCRONIZANDO`, `SESSAO_ABERTA`) | As tasks de 22/09 cobrem só `REGISTRADO`, `JA_REGISTRADO` e o feedback; os estados de rede e de sessão dependem do portal (#20). O veredito de rádio (#19) saiu em 22/09, e o `FALHA_DE_RADIO` caiu com ele (R45) | Na integração, depois da Entrega 05. **Rodada 16:** tarefa 5.11 |
| **Estado depois do login e depois do Enviar** (§5) | O pseudocódigo põe o aparelho em `SINCRONIZANDO` e não diz para qual estado ele volta: com o painel aberto, o LED fica em azul lento até o Iniciar. Caso que a decisão tem de responder: o Enviar dá certo às 20h50 e um aluno atrasado encosta às 20h51 — a sessão continua aberta? O toque grava? Um segundo Enviar seria recusado pela sessão `finalizada` (R20)? Achado em 13/09 | Na integração, depois da Entrega 05, junto da máquina de estados completa. ✅ **Fechada em R46 (23/09)**, implementada nas tarefas 5.9, 5.11 e 5.13 |
| **Perguntas 2 e 3 do portal cativo** (ticket `.scratch/presente/issues/09`): o login fica na janelinha que o celular abre sozinho ou segue no navegador; o celular que vê "rede sem internet" e troca sozinho para o 4G | A R25 diz que a escolha se mede na bancada, mas nenhuma task a testa. A #18 só **observa** e anota o que aconteceu, sem decidir. Achado em 13/09 | Quando o portal (#20) rodar dentro do aparelho, depois da Entrega 05. ✅ **Fechada em R49 (23/09)**, conferida na placa nas tarefas 5.8 e 5.16 |
| **De onde vem a turma no login** | O §6, o pseudocódigo e a R14 mandam baixar "a lista da turma", e a tela de login da #20 só pede usuário e senha. A função `roster` recebe o código da turma (R33), mas nada diz como o aparelho o obtém: escolhido pelo professor, deduzido do professor e do horário, ou gravado no aparelho. O protótipo da #20 resolveu fixando a turma no `data/mentira.js`, que é resposta de protótipo, não decisão. Achado em 13/09, na #21 | Junto do portal (#20), na integração depois da Entrega 05. ✅ **Fechada em R48 (23/09)**; o `config.h` nasce na tarefa 5.7 |
| **Contrato portal↔firmware** | As oito rotas `/api/*` e os códigos de erro do protótipo (#20) nasceram contra um arquivo de JavaScript, e subir isso a contrato congelaria um desenho que nunca foi feito contra o `WebServer`, a memória e o portal cativo. Achado em 15/09, na conferência do PR #29 | No `portal.cpp`, na integração depois da Entrega 05. **Rodada 16:** tarefa 5.2 |
| **Converter `ms_desde_boot` em `timestamp` na saída** | A R37 tirou o `timestamp` do arquivo, e o §7 continua exigindo ISO-8601 na exportação. A conta é do `clock.cpp` (§5), que ainda não existe: a hora do Iniciar menos o tempo decorrido desde o boot. Achado em 15/09, na rodada 12 | Com o portal servindo o Iniciar de verdade, depois da Entrega 05. **Rodada 16:** tarefa 5.13 |
| **O firmware integrado não pode formatar a partição** | A R41 manda tirar o `true` do `LittleFS.begin()` quando o portal e o `eventos.json` dividirem a partição, e nenhuma task de hoje toca o firmware integrado. Achado em 15/09, na conferência da #27 | Na integração, junto da montagem dos módulos do §5. **Rodada 16:** tarefa 5.7 |
...
> [!WARNING] Divergência declarada em 23/09/2026: o trabalho é livre (R52)
> Desde a rodada 16, nem a tabela acima nem as atribuições feitas nas issues valem como contrato. Cada integrante pega as issues que quiser, atribuindo-as a si e comentando antes de começar, e a linha *Requer* de cada issue diz o que ela exige. Só nascem com dono as issues que dependem de acesso ou de trabalho já em andamento: o banco com o Caio, as compras com o João, o portal e a página de histórico com o Igor. A placa continua com o João (R23), e as tarefas de rodar na placa exigem quem estiver com ela.
