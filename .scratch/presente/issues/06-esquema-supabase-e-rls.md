...
Esquema decidido em: 10/09/2026, pela issue #1 do GitHub (R20) — `alunos`, `professores`, `turmas`, `alunos_turmas`, `sessoes` (com `estado`) e `eventos`. ~~A idempotência sai do `estado`: sessão `finalizada` recusa o segundo POST, o que fecha `HU-39`.~~ — o `HU-39` fecha, mas não pelo estado: a **R33** tirou o `POST` em `sessoes` do caminho, e quem recusa o segundo envio é a chave primária `sessaoId`, com `on conflict do nothing`. O aparelho só escreve `finalizada`; `aberta` e `cancelada` ficaram para uso administrativo (**R42**, 16/09). ~~Continua em aberto a outra metade do ticket: escrever as políticas de RLS (a chave anon lê roster e insere evento, nunca apaga nem altera o passado). Dono: Caio.~~ — a outra metade fechou em 13/09 com a **R33**, que tirou o aparelho das tabelas: ele não tem privilégio em nenhuma e só executa `roster` e `enviar_relatorio`. A chave também deixou de ser a `anon`: é a **publicável** (R35). Ver a `## Answer` abaixo.
Parent: map.md
Blocked by: 03
