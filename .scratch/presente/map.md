...
**30/11/2026 — o Presente! entregue e funcionando:** protótipo físico montado, firmware, portal do professor, API na nuvem, mais a documentação completa — tendo passado por todos os 8 marcos do professor (07/09 · 14/09 · 22/09 · 29/09 · 05/10 · 20/10 · 27/10 · 30/11). ⚠️ **Em 22/09 o professor adiou a estrutura física para 06/10** e avisou que as entregas seguintes andam cerca de uma semana; as datas novas saem de cada enunciado (`docs/entregas.md`).
...
- **A execução saiu daqui em 11/09.** A quebra em epics por entrega, os vértices e o grafo de dependência vivem em `.scratch/prototipo/graph.md`, e as issues de atribuição estão no GitHub, da #5 em diante. Este mapa continua dono só dos fios de decisão que sobraram: ~~o veredito do ticket 09 e o ticket 08, reaberto pelo R19~~ os itens 2 e 3 do ticket 09, sobre o portal cativo, e o material da caixa, no ticket [04](issues/04-estrutura-fisica-material.md), retomado em 23/09 para a entrega de 06/10. O veredito saiu em 22/09 (Plano A, R45), e o ticket 08 fechou junto. A metade de RLS do ticket 06 fechou em 13/09, na #21 (R33–R35). ⚠️ **Em 23/09, os dois fios fecharam**, o 09 pela R49 e o 04 pela R53. O mapa não tem mais decisão aberta, e a execução até a entrega final vive em `.scratch/integracao/graph.md`.
...
- [Comportamento do rádio: quando sincronizar e o que acontece quando a troca falha](issues/08-comportamento-do-radio.md): sincronizar **no login**, não no Iniciar; ~~2 tentativas e depois `ESP.restart()`~~ (caiu com o Plano A, R45, 22/09); `sessao_atual.json` vira requisito para a sessão sobreviver ao reboot. A alternativa de passar a internet para o celular do professor foi descartada por razão acadêmica, não técnica.
...
- **Credencial fora do endereço da página** (item 2 do ticket [09](issues/09-retorno-ao-painel-apos-troca-de-radio.md)): a opção (c) — carregar o token na própria URL — **caiu**, por objeção do Cauã aceita na issue #3 em 10/09 e escrita na spec como **R25** em 11/09. Restam (a) manter tudo no CNA e (b) trazer o professor pelo CNA e seguir em `192.168.4.1`. ⚠️ O **veredito do ticket segue aberto** — o que fechou foi uma das três saídas, não o ticket. ✅ **Fechado em 23/09 (R49):** a opção (a), com a retomada pelo login.
- [Material e prazo da estrutura física](issues/04-estrutura-fisica-material.md): papelão ou material reaproveitado, feito pelo grupo, com um desenho, e a antena do leitor atrás de uma tampa fina e sem metal por perto (R53, 23/09). A impressora 3D e o MDF caíram. As tarefas são as #31 a #34.
- [Como o professor volta ao painel](issues/09-retorno-ao-painel-apos-troca-de-radio.md): o login fica na janela do portal cativo, e um novo login retoma a chamada em curso, porque ela vive no aparelho (R49, 23/09). A troca sozinha para o 4G se confere na placa, nas tarefas #42 e #50.
- **O trabalho é livre** ([01](issues/01-divisao-de-responsabilidades.md)): cada integrante pega as issues que quiser, e só as que dependem de acesso ou de trabalho em andamento nascem com dono (R52, 23/09).

⚠️ **Decisão revertida em 10/09:** o item "AP e STA nunca simultâneos" (R4) caiu. O João mostrou na issue #3 que `WIFI_AP_STA` é nativo, e a documentação da Espressif confirma: a restrição é de **canal** (as duas interfaces vão para o canal do STA, com CSA avisando as estações), não de impossibilidade. O impedimento que resta é de **memória** (40–50 KB do handshake TLS). O projeto passa a ter **Plano A** (simultâneo) e **Plano B** (alternância), decididos por medição na bancada — ticket [09](issues/09-retorno-ao-painel-apos-troca-de-radio.md), testes 7c e 7d do §8 da spec. Por consequência, o ticket [08](issues/08-comportamento-do-radio.md) voltou a `reopened`. ✅ **Fechado em 22/09 (R45):** vale o Plano A. A memória e o painel passaram, em iPhone e Android, e o ticket 08 voltou a `resolved`.
